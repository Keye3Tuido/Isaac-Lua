...
l local function Action(t)t=t.SpawnerEntity if t then t:ToPlayer():UseActiveItem(CollectibleType.COLLECTIBLE_WAVY_CAP,UseFlag.USE_NOANIM)end end;local A,B,E,H,M,P,T,N=Isaac.AddCallback,{},EntityType,GetPtrHash,ModCallbacks,'Position',{}A(T,M.MC_POST_FIRE_TEAR,function(_,e)B[H(e)]=e.SpawnerType==E.ENTITY_PLAYER end)A(T,M.MC_POST_TEAR_UPDATE,function(_,t)local h,m,r,a,b=H(t),math,Game():GetRoom()a=m.max b=m.abs if B[h]then for i=0,r:GetGridSize()-1 do local g,p=r:GetGridEntity(i),r:GetGridPosition(i)if g then local s,e,v=g.State,GridEntityType,g:GetType()if(v==e.GRID_POOP and s~=1e3 or v==e.GRID_TNT and s~=4)then s=t[P]+t[P..'Offset']e=a(0,b(s.X-p.X)-20)v=a(0,b(s.Y-p.Y)-20)if t.Size>=m.sqrt(e*e+v*v)then B[h]=N break end end end end end end)A(T,M.MC_PRE_TEAR_COLLISION,function(_,e)B[H(e)]=N end)A(T,M.MC_POST_ENTITY_REMOVE,function(_,e)local h=H(e)e=e:ToTear()if B[h]then Action(e)end B[h]=N end,E.ENTITY_TEAR)A(T,M.MC_POST_NEW_ROOM,function()B={}end)
