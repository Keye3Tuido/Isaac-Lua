...
l local A,B,C,E,F=Isaac.AddCallback,ModCallbacks,PickupVariant.PICKUP_COIN,GetPtrHash,{}A({},B.MC_POST_PICKUP_UPDATE,function(h,p)h=E(p)if p.Timeout>=0 and not F[h]then F[h]=1 end end,C)A({},B.MC_PRE_PICKUP_COLLISION,function(h,p,c)h=E(p)if c:ToPlayer()and F[h]then F[h]=2 end end,C)A({},B.MC_POST_ENTITY_REMOVE,function(h,e)h=E(e)if 1==F[h]then for i=1,Game():GetNumPlayers()do Isaac.GetPlayer(i-1):TakeDamage(1,0,EntityRef(e),60)end end if F[h]then h,F[h]={}for k,v in pairs(F)do if v then h[k]=v end end F=h e:Remove()end end,EntityType.ENTITY_PICKUP)
