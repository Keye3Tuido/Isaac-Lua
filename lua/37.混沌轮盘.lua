...
-- 弹速 -> 移速 -> 幸运 -> 弹速
-- 攻速 -> 射程 -> 伤害 -> 攻速
l local A,B,C,D,E,F,G,H,I,J,Z,Y,X,W,V=Isaac,ModCallbacks,CacheFlag,pairs,'MoveSpeed','MaxFireDelay','Damage','TearRange','ShotSpeed','Luck'Z=A.AddCallback Y=A.RemoveCallback X=B.MC_EVALUATE_CACHE W={[C.CACHE_SPEED]=E,[C.CACHE_FIREDELAY]=F,[C.CACHE_DAMAGE]=G,[C.CACHE_RANGE]=H,[C.CACHE_SHOTSPEED]=I,[C.CACHE_LUCK]=J}V=function(p)p:AddCacheFlags(C.CACHE_ALL)p:EvaluateItems()end Z({},B.MC_POST_PEFFECT_UPDATE,function(a,p,b)a={}for k,v in D(W)do a[k]=function()a[v]=p[v]end Z(a,X,a[k],k)end V(p)for k,_ in D(W)do Y(a,X,a[k])end b=function()p[E]=a[I]p[J]=a[E]p[I]=a[J]p[H]=1200/(a[F]+1)p[G]=a[H]/40 p[F]=30/a[G]-1 end Z(a,X,b)V(p)Y(a,X,b)end)
