...
--6. 在炼狱恶鬼被移除的位置，触发眼泪的移除效果。
l Isaac.AddCallback({},ModCallbacks.MC_POST_ENTITY_REMOVE,function(p,e)p=e.Variant==EffectVariant.PURGATORY and e.SubType==1 and e.SpawnerEntity p=p and p:ToPlayer()if p then p:FireTear(e.Position,e.Velocity,false,true):Die()end end,EntityType.ENTITY_EFFECT)

--7. 玩家的伤害值最终倍率为10%。
l Isaac.AddCallback({},ModCallbacks.MC_EVALUATE_CACHE,function(_,p)p.Damage=p.Damage/10 end,CacheFlag.CACHE_DAMAGE)
