--以下为数据保存的可读多行源代码，已注释化，仅保留作参考；实际使用下方单行压缩版（模板 persistent-data）。
--[[ 可读源代码（非 YAML 头）
...
说明: 用于储存数据，无实际效果。_Data()返回的表兼容发光沙漏和rewind（可回溯），_Data(entity)=entity:GetData()（可回溯），_rew()返回当前是否处于rew状态（不要更新表中的数据），_Pata()返回的数据仅在游戏结束时重置（不回溯）。
]]
