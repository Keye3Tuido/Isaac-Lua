...
**sound 性边界（177 → 169 暂不实现）**：理想 169 需对索引表达式 `a[c]` 做冗余存储消除，但 `a[c]` 可能触发 `__index` 副作用、静态不可证纯，不属于可机器证明等价的变换。`canonical(177 形)===canonical(173 形)` 返回 false（归一正确拒绝判等）。详见 `OPTIMIZATION_TODO.md`。
...
177 是"只做可机器证明等价变换"原则下的安全最优；剩余 8 字需对索引表达式做冗余存储消除（不 sound），详见 `OPTIMIZATION_TODO.md`。
