...
1.4d 块包装（重复语句块打包成薄函数：固定点任意位置、变化点作形参；散布重复也命中，贪心取语句不重叠的最短分解）
