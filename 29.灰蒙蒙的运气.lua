...
--1. 从道具池抽取道具时，按照等级从低到高抽取
l local f,Z=Isaac f.AddCallback({},ModCallbacks.MC_PRE_GET_COLLECTIBLE,function(a,z,y,x,b,c,d,e)if not Z then b=f.GetItemConfig()a=Game():GetItemPool()d='GetCollectible'for i=1,5 do e=b[d..'s'](b).Size if i<5 then repeat e=e-1 c=b[d](b,e)if c and i<=c.Quality then a:AddRoomBlacklist(e)end until e<0 and not c end Z=true c=b[d](b,a[d](a,z,y,x))e=c.ID Z=nil a:ResetRoomBlacklist()if CollectibleType.COLLECTIBLE_BREAKFAST~=e then return e end end end end)
