...
l local F,I,J,K,L,G,H,D,M,O,S,T,U,V,W,X,Y,Z,E,Q,R,C,A,B,P=1,Isaac,Game,true,false,function(p)return tostring(p:GetCollectibleRNG(1):GetSeed())end,Sprite(),FamiliarVariant,ModCallbacks,CacheFlag.CACHE_DAMAGE,{},{},'GetPlayerType','Parent','Position','Player','ACTION_SHOOT',Vector,'Visible','INCUBUS','KING_BABY','Collectible'A,B,P=I.AddCallback,'Add'..C,I.GetPlayer;H:Load('gfx/1000.185_redemption.anm2',K)H:Play('Idle',K)A(T,M.MC_POST_GAME_STARTED,function(_,c)F=1 if not c then S={}end end)A(T,M.MC_POST_UPDATE,function(m,t,d,i,p,h,u)if Y then J():GetHUD():AssignPlayerHUDs()end m,Y=P()d,i,t=0,1,m[U](m)while d<6 do p=P(i)h=G(p)if h==G(m)then I.ExecuteCommand('addplayer '..t)p=P(i)Y,h=K,G(p)end u=p[U](p)if u<PlayerType.NUM_PLAYER_TYPES then d=d+1 if not S[h]then S[h]={n=i,p=p,x=m[W]}S[i]=S[h]p:AddCacheFlags(O)p:EvaluateItems()end end i=i+1 end end)A(T,M.MC_POST_PLAYER_UPDATE,function(u,p,m,o,y,z)m=P()u,o,z=G(p),_G[C..'Type'],'COLLECTIBLE_'y=S[u]if y or u==G(m)then if not p:HasCurseMistEffect()then for k,v in pairs{[L]=o[z..Q],[K]=o[z..R]}do if not p['Has'..C](p,v)and(y or k)then p[B](p,v)end end end if y then if y.n==F then p[V]=nil p[B](p,7)p['Remove'..C](p,7)end y.p,p.Size,p[E],p[V],y.x,p[W]=p,0,L,m,50*Z.FromAngle(60*y.n),m[W]p:SetMinDamageCooldown(1)z=m[U](m)if z~=p[U](p)then p:ChangePlayerType(z)end elseif not p:IsItemQueueEmpty()then z,y=p.QueuedItem,'Item'if not z.Touched and z[y]['Is'..C](z[y])then m,y=z[y].ID,S[F].p y[B](y,m)p:FlushQueueItem()end end end end)A(T,M.MC_INPUT_ACTION,function(x,e,h,a)e=e and e:ToPlayer()x=e and G(e)if e and(S[x]or x==G(P()))then if h==InputHook.GET_ACTION_VALUE then e,h=1,0 else e,h=K,L end if a//4==1 then return a==4+J():GetFrameCount()%4 and e or h elseif S[x]then return h end end end)A(T,M.MC_FAMILIAR_UPDATE,function(p,f,v)v,p=f.Variant,P()[W]if v==D[R]then f[E],f[W]=L,p else f:FollowPosition((v==D[Q]and 1 or 2)*S[G(f[X])].x+p)end end)A(T,M.MC_POST_FAMILIAR_RENDER,function(_,f,o)o=o+I.WorldToRenderPosition(f[W])if S[G(f[X])].n==F then H:Render(o+Z(0,20))if 1>1&I.GetFrameCount()then H:Update()end end end,D[Q])A(T,M.MC_POST_RENDER,function()if Input.IsActionTriggered(ButtonAction.ACTION_DROP,P().ControllerIndex)then F=F%6+1 end end)A(T,M.MC_EVALUATE_CACHE,function(d,p)if S[G(p)]then d='Damage'p[d]=p[d]/22.5 end end,O)
