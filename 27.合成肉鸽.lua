...
l local F,I,J,K,L,G,H,D,M,O,S,T,U,V,W,X,Z,E,Q,R,C,B,A,P,N,Y=1,Isaac,Game,true,false,function(p)return tostring(p:GetCollectibleRNG(1):GetSeed())end,'MAP',FamiliarVariant,ModCallbacks,CacheFlag.CACHE_DAMAGE,{},{},'GetPlayerType','Parent','Position','Player',Vector,'Visible','INCUBUS','KING_BABY','Collectible','Add'A,P,N,T.S=I.AddCallback,I.GetPlayer,function(b,x)return Input['IsAction'..(x and'Triggered'or'Pressed')](ButtonAction['ACTION_'..b],P().ControllerIndex)end,Sprite()T.S:Load('gfx/1000.185_redemption.anm2',K)T.S:Play('Idle',K)A(T,M.MC_POST_GAME_STARTED,function(_,c)F=1 if not c then S={}end end)A(T,M.MC_POST_UPDATE,function(m,d,i,p,h,u)if Y then J():GetHUD():AssignPlayerHUDs()end m,Y=P()d,i=0,1 while d<6 do p=P(i)h=G(p)if h==G(m)then I.ExecuteCommand('addplayer '..m[U](m))p=P(i)Y,h=K,G(p)end u=p[U](p)if u<PlayerType.NUM_PLAYER_TYPES then d=d+1 S[h]={n=d,p=p,x=50*Z.FromAngle(60*d)}S[i]=S[h]if Y then p:AddCacheFlags(O)p:EvaluateItems()end end i=i+1 end end)A(T,M.MC_POST_PLAYER_UPDATE,function(u,p,m,o,y,z,b,t)m=P()u,o,b,z=G(p),_G[C..'Type'],'Add'..C,'COLLECTIBLE_'y=S[u]if y or u==G(m)then if not p:HasCurseMistEffect()then for k,v in pairs{[L]=o[z..Q],[K]=o[z..R]}do if not p['Has'..C](p,v)and(y or k)then p[b](p,v)end end end o='Remove'..C if y then if y.n==F then p[V]=nil p[b](p,7)p[o](p,7)end y.p,p.Size,p[E],p[V],p[W],o=p,0,L,m,m[W],'BrokenHearts'z=B..o p:SetMinDamageCooldown(1)p[z](p,1e3)p[z](p,-p['Get'..o](p))p:AddMaxHearts(2)p:AddHearts(2)p:Revive()z=m[U](m)if z~=p[U](p)then p:ChangePlayerType(z)end elseif not p:IsItemQueueEmpty()then z=p.QueuedItem y=z.Item if y.Type~=ItemType.ITEM_ACTIVE and y['Is'..C](y)and not y:HasTags(ItemConfig.TAG_QUEST)then t,m,y=p.FlushQueueItem,y.ID,S[F].p if N(H)then t(p)else y[b](y,m,0,z.Touched)t(p)p[o](p,m)end end end end end)A(T,M.MC_INPUT_ACTION,function(x,e,h,a)e=e and e:ToPlayer()x=e and G(e)if e and(S[x]or x==G(P()))then if h==InputHook.GET_ACTION_VALUE then e,h=1,0 else e,h=K,L end if a//4==1 then for _,v in pairs{'LEFT','RIGHT','UP','DOWN'}do if N('SHOOT'..v)then return a==4+J():GetFrameCount()%4 and e or h end end elseif not S[x]then return end return h end end)A(T,M.MC_FAMILIAR_UPDATE,function(p,f,v,s)v,p,s=f.Variant,P()[W],S[G(f[X])]if v==D[R]then f[E],f[W]=L,p elseif s then if v==D[Q]then f:AddVelocity(s.x+p-f[W])else f:FollowPosition(2*s.x+p)end end end)A(T,M.MC_POST_FAMILIAR_RENDER,function(s,f,o)o,s=o+I.WorldToRenderPosition(f[W]),S[G(f[X])]if s then if N(H)then f[E]=L return else f[E]=K end if s.n==F then T.S:Render(o+Z(0,20))if 1>1&I.GetFrameCount()then T.S:Update()end end end end,D[Q])A(T,M.MC_POST_RENDER,function()if N('DROP',K)and not N(H)and not J():IsPaused()then F=F%6+1 end end)A(T,M.MC_EVALUATE_CACHE,function(d,p)if S[G(p)]then d='Damage'p[d]=p[d]/22.5 end end,O)A(T,M.MC_PRE_PLAYER_COLLISION,function(_,p)if S[G(p)]then return K end end)

--3. 从游戏中移除道具122(巴比伦大淫妇)、482(遥控器)
l local I,C,Y,T,A=Isaac,{122,482},true,{}A=I.AddCallback A(T,23,function(_,c)for _,v in pairs(C)do if c==v then return Y end end end)A(T,31,function(_,p)for _,i in pairs(C)do for _=1,p:GetCollectibleNum(i)do p:RemoveCollectible(i)end end end)A(T,37,function(p,f,v,s)if v==100 then repeat p,f=Game():GetItemPool()for _,i in pairs(C)do if i==s then f,s=1,p:GetCollectible(p:GetLastPool(),Y)break end end until not f return{v,s}end end)
